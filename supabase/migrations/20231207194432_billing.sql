...
(
  ...
  workspace_id  uuid references workspaces not null primary key,
  ...
  provider    billing_providers default 'stripe'::billing_providers
);
...
create policy "Can only view own customer data." on billing_customers for
  ...
  using (
    workspace_id IN (
      SELECT saas.get_workspaces_for_current_user() AS get_workspaces_for_current_user
    )
  );
...
CREATE OR REPLACE FUNCTION public.get_workspace_billing_status(lookup_workspace_id uuid)
    RETURNS json AS
$$
DECLARE
    result RECORD;
BEGIN
    ...
    where s.workspace_id = lookup_workspace_id
    order by s.created desc
    limit 1
    into result;

    if result is null then
        raise 'No billing data found for workspace %', lookup_workspace_id;
    end if;
...
grant execute on function public.get_workspace_billing_status(uuid) to authenticated, service_role;
